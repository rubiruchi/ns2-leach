...
MacSensor::command(int argc, const char*const* argv)
{
  if (argc == 3) {
    ...
    if(strcmp(argv[1], "nodes") == 0) {
      ...
      printf("Asserting the Cache is present\n"); 
      assert(cache_);
...
MacSensor::recv(Packet *p, Handler *h)
{
  ...
  if (num_codes > ss_) {
    ...
    fflush(stdout);
    /******* Added by Jason Pamplin to fix bug where rx_state_ was not
       set prior to calling a collision.  If a collission has occurred
       then we should se the packet type to be MAC_COLL *******/
...
MacSensor::collision(Packet *p)
{
  ...
  case MAC_COLL:
    ...
    break;
  /*********Jason A. Pamplin
     Adding this code to prevent failure if a collision registers
   when the node is idle anyway.  In this case, it should do nothing 
